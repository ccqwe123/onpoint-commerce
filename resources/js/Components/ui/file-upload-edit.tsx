...
interface FileUploadProps {
  ...
  onFilesReordered: (files: (UploadedFile | ProductImage)[]) => void;
  uploadedFiles: UploadedFile[];
  existingImages?: ProductImage[];
  onExistingImageRemoved?: (imageId: number) => void;
  accept?: string;
...
//   const allImages = [...existingImages, ...uploadedFiles];
  ...
  const [allImages, setAllImages] = useState<(ProductImage | UploadedFile)[]>([
    ...existingImages,
    ...uploadedFiles,
    ]);
  useEffect(() => {
    setAllImages([...existingImages, ...uploadedFiles]);
      ...
      {allImages.length > 0 && (
        <div className="space-y-2">
          {allImages.map((image, index) => {
            const isExisting = 'product_id' in image;
            const isUploaded = 'file' in image;
            
            return (
              <div
                key={isExisting ? `existing-${image.id}` : `uploaded-${image.id}`}
                draggable
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-sm font-medium text-foreground truncate">
                      {isExisting ? `Image ${image.id}` : image.file.name}
                      {isExisting && image.is_primary === 1 && (
                  ...
                  {isUploaded && image.status === 'uploading' && (
                  ...
                  {isUploaded && image.status === 'completed' && (
                ...
                <Button
                  ...
                  onClick={(e: React.FormEvent) => {
                    e.preventDefault();
                    if (isExisting) {
                      onExistingImageRemoved?.(image.id);
                    } else {
                      onFileRemoved(image.id);
                    }
                  }}
